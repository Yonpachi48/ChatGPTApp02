...
extension OpenAISwift {
    ...
    ///   - completionHandler: Returns an OpenAI Data Model
    public func sendImages(with prompt: String, model: OpenAIModelType = .image(.dalle3), numImages: Int = 1, size: ImageSize = .size1024, user: String? = nil, completionHandler: @escaping (Result<OpenAI<UrlResult>, OpenAIError>) -> Void) {
        let endpoint = OpenAIEndpointProvider.API.images
        let body = ImageGeneration(prompt: prompt, model: model.modelName, n: numImages, size: size, user: user)
        let request = prepareRequest(endpoint, body: body)
